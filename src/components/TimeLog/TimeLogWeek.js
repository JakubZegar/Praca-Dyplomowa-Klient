...
function TimeLogWeek(apiKey, userDetails) {
    ...
    useEffect(() => {
        for(let i = 0; i < 7; ++i) {
            ...
            setTimeLogWeekData((timeLog) =>{
                return [...timeLog, [convertDateToMySqlFormat(date), 0]];
            })
        }
        let newTimeLogValue = []
        lastWeekLogs.map((log) => (
            timeLogWeekData.map((timeLog, index) => (
                (new Date(log.date).getDay() === new Date(timeLog[0]).getDay()) ? (
                    newTimeLogValue = [...timeLogWeekData],
        ...
        setSeries( () => {
            return new TimeSeries({
                ...
                points: timeLogWeekData.map(([d, value]) => [
    ...
    const style = styler([
        { key: "precip", color: "#BF567D", display: "flex",},
    ]);
    ...
    return (
        <>
        {
            is24hDataReady === true ?
            <TimeLogPlotBg>
                <TimeLogPlotHeader>{t('lastWeek')}</TimeLogPlotHeader>
                <Resizable>
                    <ChartContainer 
